...
void Client::parse_header_end(const QByteArray &u) {
    if (u.startsWith(")")) {
        QRegExp re("\"(.*)\"\\s*<(.*)>");
        QString ffrom;
        if (re.indexIn(from) > -1) {
            if (re.cap(1) == re.cap(2))
                ffrom = re.cap(1);
            else ffrom = from;
        }

        QStringList dl = QString(date).split(" ", QString::SkipEmptyParts);
        QString fdate("- (%1 %2, %3) - ");
        headers.append(fdate.arg(dl.at(1), dl.at(2), dl.at(4)) + subject + " - " + ffrom + "\n" );
